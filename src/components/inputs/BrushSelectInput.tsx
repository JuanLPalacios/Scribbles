    ...
    return <>
        <TopMenuPortal>
            <div style={style} className='brush dropdown'>
                <button>
                    <Canvas canvas={selectedPreview.canvas} />Brush
                </button>
                <ul>
                    {previews.map(({ canvas }, i) => <li key={id+'-'+i}><Canvas canvas={canvas} className={i==selectedBrush ? 'selected' : ''} onMouseDown={()=>onChange({ ...props, selectedBrush: i })} /></li>)}
                </ul>
            </div>
        </TopMenuPortal>
        <LeftMenuPortal>
