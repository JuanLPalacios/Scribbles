...
import { Serialized, SerializedValue, CompressedValue, Compressed, isSerializedImageData, isSerialized, isCompressedImageData, isCompressedJSON } from './Serialization';
import { serializeImageData, deserializeImageData, serializeDrawingState, deserializeDrawingState } from './serializeJSON';
import { DrawingState } from '../contexts/DrawingContext';

export type SbrVersionInfo = {
    version:number
    subVersion:number
};
                ...
                zip.files['version.json'].async('string')
                    ...
                    .then((version:SbrVersionInfo)=>
                        zip.files['content.json'].async('string')
                            .then(JSON.parse)
                            .then(async (zippedBrush:Compressed)=>{
                                ...
                                return deserializeDrawingState(brush, CANVAS, CTX);
                            })
                    )
            );
    },
    ...
    async binary(state: DrawingState) {
        ...
        for (const key in data) {
            if (Object.prototype.hasOwnProperty.call(data, key)) {
                zippedBrush[key] = await zipDataV1S1(data[key], context);
            }
        }
        zip.file('content.json', JSON.stringify(zippedBrush));
        return zip.generateAsync({ type: 'blob' });
    }
};

export function zipDataV1S1(value: SerializedValue, context: Context):Promise<CompressedValue> {
    return (async ():Promise<CompressedValue> => {
        if(typeof value !==  'object')return value;
        if(Array.isArray(value))return Promise.all(value.map(x=>zipDataV1S1(x, context)));
        if(isSerializedImageData(value)) {
            if(!CTX) return '';
        ...
        if(isSerialized(value)){
            ...
            for (const key in value) {
                if (Object.prototype.hasOwnProperty.call(value, key)) {
                    zippedBrush[key] = await zipDataV1S1(value[key], context);
                }
            }
            return zippedBrush;
        }
...
export function getUnzipDataFunction({ version, subVersion }: SbrVersionInfo): ((value: CompressedValue, context: Context) => Promise<SerializedValue>) {
    switch (version) {
    case 1:
...
export function getUnzipDataV1(subVersion : number): ((value: CompressedValue, context: Context) => Promise<SerializedValue>) {
    switch (subVersion) {
    case 1:
...
export function unzipDataV1S1(value : CompressedValue, context:Context): Promise<SerializedValue> {
    return new Promise<SerializedValue>((resolve, reject) => {
        ...
        if(isCompressedImageData(value)){
        ...
        if(isCompressedJSON(value))return resolve(value);
        return resolve((async (compressed:Compressed)=>{
            if(!CTX) throw new Error();
            const
                serialized:Serialized = {};
            for (const key in compressed) {
                if (Object.prototype.hasOwnProperty.call(compressed, key)) {
                    serialized[key] = await unzipDataV1S1(compressed[key], context);
                }
            }
            return serialized;
        })(value));
...
export function dataURLFormat(contentType: string, base64: string): string {
